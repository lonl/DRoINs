...
def _get_blobs(im1, im2, rois):
    ...
    if cfg.TEST.HAS_RPN:
        ...
        blobs['data1'], im_scale_factors = _get_image_blob(im1)
        blobs['data2'], _ = _get_image_blob(im2)
    # else:
...
def im_detect(sess, net, im1, im2, name, boxes=None):
    ...
    # forward pass
    if cfg.TEST.HAS_RPN:
        feed_dict={net.data1: blobs['data1'], net.data2: blobs['data2'],net.im_info: blobs['im_info'], net.keep_prob: 1.0}
    else:
        feed_dict={net.data: blobs['data'], net.rois: blobs['rois'], net.keep_prob: 1.0}

    rois, scores1, score = \
        sess.run([net.get_output('crp'), net.score1, net.get_output('rpn_cls_prob_reshape')],
                 feed_dict=feed_dict)
    ...
    # pred_boxes = _clip_boxes(pred_boxes, im1.shape)
    # else:
    ...
    print score.shape
    feat = score[0, :, :,9:]
    vis_square(feat, '{}.png'.format(name), padval=1)
...
def vis_square(data, fn, padsize=1, padval=0):
    ...
    c = fig2.add_subplot(1,1,1)
    print data.shape
    c.imshow(data)
    c.axis('off')
    fig2.savefig('/home/a409/users/liqing/diff_v2/featuremap/'+fn)
    fig2.clf()
