...
def prepare_roidb(imdb):
    ...
    """
    if imdb.flipped:
       imdb.picture_name = imdb.picture_name * 2
    roidb = imdb.roidb
    sizes = [PIL.Image.open(os.path.join(imdb.image_path_at(i),imdb.picture_name[i][0])).size
             for i in xrange(imdb.num_images)]
    for i in xrange(len(imdb.image_index)):
        roidb[i]['image1'] = os.path.join(imdb.image_path_at(i),imdb.picture_name[i][0])
        roidb[i]['image2'] = os.path.join(imdb.image_path_at(i),imdb.picture_name[i][1])
