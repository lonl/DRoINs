...
from ..change_detection.crp import crp_layer as crp_layer_py
from ..change_detection.change_target_layer import change_target_layer as change_target_layer_py
...
class Network(object):
    ...
    def load(self, data_path, session, ignore_missing=False):
        ...
        for key in data_dict:
            with tf.variable_scope("siamese/"+key, reuse=True):
    ...
    @layer
    def deconv(self, input, out_channels, k, s_h, s_w, name):
        initializer = tf.random_normal_initializer(0, 0.02)
        return tf.layers.conv2d_transpose(input, out_channels, kernel_size=k, strides=(s_h, s_w), padding=DEFAULT_PADDING,
    ...
    def proposal_layer(self, input, _feat_stride, anchor_scales, cfg_key, name):
        ...
        roi, score = tf.py_func(proposal_layer_py,\
                                     [input[0],input[1],input[2], cfg_key, _feat_stride, anchor_scales],\
                                     [tf.float32, tf.float32])
        roi = tf.reshape(roi, [-1,5],name =name)
    ...
    def anchor_target_layer(self, input, _feat_stride, anchor_scales, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            print input[0],input[1],input[2],input[3],input[4], _feat_stride, anchor_scales
    ...
    @layer
    def change_target_layer(self, input, name):
        if isinstance(input[0], tuple):
            input[0] = input[0][0]
        with tf.variable_scope(name) as scope:
            rois, labels, bbox_targets, bbox_inside_weights = tf.py_func(change_target_layer_py, [input[0], input[1]], [tf.float32, tf.float32, tf.float32, tf.float32])
            rois = tf.reshape(rois, [-1, 5], name='rois')  # goes to roi_pooling
            labels = tf.convert_to_tensor(tf.cast(labels, tf.float32), name='labels')
            bbox_targets = tf.convert_to_tensor(bbox_targets, name = 'bbox_targets')
        return rois, labels, bbox_targets, bbox_inside_weights


    @layer
    def crp(self, input, name):
        if isinstance(input[0], tuple):
            input[0] = input[0][0]

        with tf.variable_scope(name) as scope:
            roi = tf.py_func(crp_layer_py, [input[0], input[1], input[2], input[3]], [tf.float32])
            roi = tf.reshape(roi, [-1, 5], name='rois')
            self.layers['rois_c'] = roi
        return roi
    ...
    def build_loss(self, ohem=False):
        ...
        # bbox_targets = self.get_output('change_data')[2] # (R, (C+1)x4)
        # bbox_inside_weights = self.get_output('change_data')[3]
        ...
        ####### siamese
        # margin = 5.0
        # labels_t = self.get_output('change_data')[1]
        # labels_f = tf.subtract(1.0, labels_t , name="1-yi")  # labels_ = !labels;
        # eucd2 = tf.pow(tf.subtract(self.score1, self.score2), 2)
        ...
        # loss_diff = tf.reduce_mean(losses, name="loss")

        EPS = 1e-12
        #####generator_loss
        gen_loss_GAN = tf.reduce_mean(tf.log(1 - self.score2 + EPS))
        gen_loss_L1 = tf.reduce_mean(tf.abs(self.fakes - self.targets))
       # gen_loss = 0.1 * gen_loss_GAN +  10 * gen_loss_L1
        gen_loss = 0.1 * gen_loss_GAN

        #####discriminator_loss
        discrim_loss = tf.reduce_mean(-(tf.log(self.score1 + EPS) + tf.log(1 - self.score2 + EPS)))

        loss = rpn_cross_entropy + rpn_loss_box

        return loss, rpn_cross_entropy, rpn_loss_box, gen_loss, discrim_loss
